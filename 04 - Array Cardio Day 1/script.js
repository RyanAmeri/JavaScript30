    ...
    const sumAges = (arr) => {
        return arr.reduce((sum, inventor,) => {
            return sum + (inventor.passed - inventor.year)
        }, 0);
    };
    console.log(sumAges(inventors));

    // 5. Sort the inventors by years lived

    const yearsLived = (arr) => {
        newArr = [...arr];
    ...
    const countData = (arr) => {
        return arr.reduce((count, item) => {
            count.has(item) ? count.set(item, count.get(item) + 1) : count.set(item, 0);
            return count;
        }, new Map());
    };
